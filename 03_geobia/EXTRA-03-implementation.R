...
segm_list <- map(
  list.files("00_data/03-geobia/ms_tiles_stats_nir/", full.names = TRUE),
  \(file) open_dataset(file) |> st_as_sf()
)

## modelo
best_model <- read_rds("00_data/03-geobia/best_ranger_nir.rds")
...
## exportar
writeRaster(preds_sr, "00_data/03-geobia/predicciones-ranger-nir.tiff", overwrite = TRUE)
