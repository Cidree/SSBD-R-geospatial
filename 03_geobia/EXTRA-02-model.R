...
annotated_data_sf <- read_sf("00_data/03-geobia/training-points-stats-nir.gpkg")
...
norm_rec <- recipe(class ~ ., data = train_tbl) |> 
  ...
  step_rm(starts_with("max"), median.R, median.G, median.B, stdev.NIR, stdev.G, stdev.B, min.R, min.G, min.ndvi)
...
write_rds(test_wflow, "00_data/03-geobia/best_ranger_nir.rds")
